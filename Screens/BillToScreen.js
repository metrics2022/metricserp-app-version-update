...
const BillToScreen = ({ navigation, route }) => {
    const cartState = useSelector(state => state.CartReducer);
    const addressState = useSelector(state => state.AddressReducer);
    const [customerSiteId, setCustomerSiteId] = useState('');

    useEffect(() => {
        // console.log('allAdreess', addressState.allBillToAddress);
        if (addressState.allBillToAddress.length > 0) {
            setCustomerSiteId(addressState.allBillToAddress[0].customer_site_id);
        }
    }, [addressState]);
    ...
    return (
        <>
            <View style={styles.mainWrapper}>
                ...
                <ScrollView >
                {
                    addressState.allBillToAddress.length > 0 ? (
                        addressState.allBillToAddress?.map((elem, index, arr) => {
                            return (
            ...
            </View>
            {
                cartState.cartItems.length > 0 && (
                    <View style={styles.checkOutBtn}>
                        ...
                        <View>
                            <TouchableOpacity style={styles.btnCheckout} onPress={() => navigation.navigate('Shipto', {
