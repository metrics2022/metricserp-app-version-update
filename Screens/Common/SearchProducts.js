...
const SearchProducts = ({ route, navigation }) => {
    ...
    //Cagtegory serach and loadmore handler
    

    const handleCategorySearch = async (value, loadMore = false) => {
        if (categoryLoader && !loadMore) return;
        
        setCategoryLoader(true);
        const val = await getMyLocalData();

        const pageToUse = loadMore ? pageNumberOfCategory : 1;
        ...
        try {
            ...
            if (loadMore) {
                const existingIds = new Set(categoryList.map(item => item.category_id));
                const uniqueNewCategories = newCategories.filter(item => 
                    !existingIds.has(item.category_id)
                );
                setCategoryList(prevList => [...prevList, ...uniqueNewCategories]);
            } else {
                setCategoryList(newCategories);
            }
            
            setCategoryTotalCount(response?.data?.data?.total_count || 0);
        } catch (error) {
    ...
    const handleCategoryLoadMore = () => {
        ...
        if (!loader && categoryList?.length < categoryTotalCount && categoryTotalCount > 0) {
    ...
    useEffect(() => {
        ...
        if (pageNumberOfCategory > 1 && !loader) {
            handleCategorySearch(inputVal, true);
        }
    }, [pageNumberOfCategory]);
    //.................................//


    //Brand search and loadmore handler

    const handleBrandSearch = async (value, loadMore = false) => {
        console.log("Brand Search Value: ", 1);
        if (brandLoader && !loadMore) return;
        
        setBrandLoader(true);
        const val = await getMyLocalData();

        const pageToUse = loadMore ? pageNumberOfBrand : 1;
        ...
        try {
            ...
            if (loadMore) {
                const existingIds = new Set(bandList.map(item => item.brand_id));
                const uniqueNewBrands = newBrands.filter(item => 
                    !existingIds.has(item.brand_id)
                );
                setBrandList(prevList => [...prevList, ...uniqueNewBrands]);
            } else {
                setBrandList(newBrands);
            }
            
            setBrandTotalCount(response?.data?.data?.total_count || 0);
        } catch (error) {
    ...
    const handleBrandLoadMore = () => {
        if (!loader && bandList?.length < brandTotalCount && brandTotalCount > 0) {
            setPageNumberOfBrand(prevPage => prevPage + 1);
        }
    }
   
    useEffect(() => {
        if (pageNumberOfBrand > 1 && !loader) {
            handleBrandSearch(brandInputVal, true);
        }
    }, [pageNumberOfBrand]);
    ...
    const handleOpen = (item) => {
        ...
        // Set item data
        setItemDesc(item?.item_description || 'No description');
        setProductId(item?.item_id || '');
        setItemLineDesc(item?.item_description_details || '');
        
        // Handle pricing data
        const prices = item?.cpl_price?.length > 0 ? item.cpl_price : item?.price || [];
        setPricingArray(prices);
        setCplLine(item?.cpl_price || []);
        
        if (prices.length > 0) {
            const firstPrice = prices[0];
            setUom(firstPrice?.uom_id || '');
            setUom_name(firstPrice?.measure_name || '');
            setPrice(firstPrice?.unit_price?.toString() || '0');
            setMarketPrice(firstPrice?.market_price?.toString() || '');
            setTotalPrice(firstPrice?.unit_price?.toString() || '0');
        } else {
            // Default values if no prices
            setUom('');
            setUom_name('');
            setPrice('0');
            setMarketPrice('');
        ...
        return (
            options?.map((item, index) => (
                <Chip 
                    ...
                    selectedColor={uom == item.uom_id ? "#FFF" : "#2C3E50"} 
    ...
    const renderItem = ({ item }) => {
        // Check if item exists in cart directly from Redux state
        const itemInCart = cartState.cartItems.some(cartItem => 
            cartItem.productId == item.item_id
        );
        ...
        return (
            <TouchableOpacity 
                ...
                onPress={() => {
                    // console.log('Product pressed:', item.item_id);
                    if (itemInCart) {
                        // console.log('Item in cart, navigating to cart');
                        handlePress();
                    } else {
                        console.log('Item not in cart, opening modal');
                        handleOpen(item);
                    }
                }}
                activeOpacity={0.7}
            >
                ...
                <View style={styles.imageContainer}>
                    {item.item_image != "" ? (
                        ...
                        <Image 
                            source={{ uri: "https://st3.depositphotos.com/23594922/31822/v/600/depositphotos_318221368-stock-illustration-missing-picture-page-for-website.jpg" }} 
                            style={styles.productImage} 
                        />
                    )}
                </View>
                ...
                <View style={styles.productInfo}>
                    ...
                    <Text style={styles.productPrice}>
                        ...
                        {item?.cpl_price?.length > 0 
                            ? Number(item?.cpl_price[0]?.market_price || 0).toFixed(2) 
                            : Number(item?.price[0]?.unit_price || 0).toFixed(2)}
                        <Text style={styles.unitText}>
                            /{item?.cpl_price?.length > 0 
                                ? item?.cpl_price[0]?.measure_name 
                                : item?.price[0]?.measure_name}
                        </Text>
                    </Text>
                    ...
                    <TouchableOpacity 
                        style={[
                            styles.cartButton,
                            itemInCart && styles.goToCartButton
                        ]}
                        onPress={() => {
                            if (itemInCart) {
                                handlePress();
   ...
   const RenderCategoryItem = React.memo(({ item }) => {
        return (
            <TouchableOpacity 
                onPress={() => { 
                    ...
                    setInputVal(""); 
                    setPageNumberOfCategory(1); // Reset page number
    ...
    const renderCategoryFooter = () => {
        if (categoryLoader && categoryList?.length < categoryTotalCount) {
            return (
        ...
        if (!categoryLoader && categoryTotalCount > 0 && categoryList?.length >= categoryTotalCount) {
            return (
    ...
    return (
        <TouchableOpacity 
            onPress={() => { 
                ...
                setBrandModal(false); 
                setBrandInputVal(""); 
                setPageNumberOfBrand(1); // Reset page number
 ...
 const renderBrandFooter = () => {
    if (brandLoader && bandList?.length < brandTotalCount) {
        return (
            <View style={styles.loadingFooter}>
                <ActivityIndicator size="small" color="#1788F0" />
    ...
    if (!brandLoader && brandTotalCount > 0 && bandList?.length >= brandTotalCount) {
        return (
            <View style={styles.endOfList}>
                                    ...
                                    <View style={styles.modalActions}>
                                        <Pressable
                                            ...
                                            onPress={() => {
                                                ...
                                                // Properly set that this item is now in cart
                                                setIsItemExistCart((prev) => ({
                                                    ...prev,
                                                    [productId]: true
                                                }));
                    ...
                    <View style={styles.modalHeader}>
                        ...
                        <View style={styles.modalSearchContainer}>
                           <TextInput 
                                ...
                                onChangeText={(e) => { 
                                    setInputVal(e);
                                    setPageNumberOfCategory(1); // Reset to page 1
                    ...
                    <View style={styles.modalHeader}>
                        ...
                        <View style={styles.modalSearchContainer}>
                           <TextInput 
                                ...
                                onChangeText={(e) => { 
                                    setBrandInputVal(e);
                                    setPageNumberOfBrand(1); // Reset to page 1
...
const styles = StyleSheet.create({
    ...
    // Modal styles (ORIGINAL DESIGN - FIXED)
    // Modal styles (ORIGINAL DESIGN - FIXED)
centeredView: {
    flex: 1,
    justifyContent: 'center',
