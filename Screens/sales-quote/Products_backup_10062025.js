...
const Products = ({ route, navigation }) => {
    ...
    const handleBrandSearch = async (value) => {
        ...
        try {
            ...
            // console.log("response?.data?.data?.brands", response?.data?.data?.brands)
            setLoader(false);
    ...
    const handleCategorySearch = async (value) => {
        ...
        try {
            ...
            // console.log("response?.data?.data?.categories", response?.data?.MessageCode)
            setLoader(false);
    ...
    // console.log("searchState?.brandId", searchState?.brandId);
    // console.log("searchState?.searchAllValues", searchState?.searchAllValues)
    ...
    const handleSearch = useCallback(() => {
        ...
        dispatch(Search(data));
        // let saveSearchData = {
        //     catName: categoryNameForShow,
        //     brandName: brandNameForShow,
        //     brandId: brandId,
        //     categoryId: categoryId,
        //     value: searchKeyword,
        // }
        // dispatch({ type: 'SAVE_SEARCH', payload: saveSearchData })
        setPage(1);
        Keyboard.dismiss();
    ...
    const handleOpen = (item) => {
        ...
        // let existed_item = cartState.cartItems.find(item => (itemid === item.productId));
        // console.log('hhhhhhhh', existed_item);
        // if (existed_item) {
        //     setModalVisible(true);
        //     setItemDesc(existed_item.itemDesc);
        //     setPricingArray(pricing);
        //     setUom(existed_item.uom);
        //     setUom_name(existed_item.uom_name);
        //     setPrice(pricing[0].item_price);
        //     setTotalPrice(existed_item.totalPrice);
        //     setProduct_qty(existed_item.product_qty);
        //     setProductId(existed_item.productId);
        // } else {
        //     setModalVisible(true);
        //     setItemDesc(desc);
        //     setPricingArray(pricing);
        //     setUom(pricing[0].measure_id);
        //     setUom_name(pricing[0].measure_name);
        //     setPrice(pricing[0].item_price);
        //     setTotalPrice(pricing[0].item_price);
        //     setProduct_qty(1);
        //     setProductId(itemid);
        // }

        // console.log("item", item?.cpl_price);
        setModalVisible(true);
    ...
    useEffect(() => {
        if (product_qty > 0) {
            ...
            // console.log('amount-area', product_qty);
        } else {
            setProduct_qty(1);
            setTotalPrice(price);
        }
    ...
    const handleRadioBtn = (measureName, measureId, itemPrice, index) => {
        ...
        // let existed_item = cartState.cartItems.find(item => (productId === item.productId && uom === item.uom));

        // console.log('sdddgg', cartState.cartItems)
        ...
        return (
            <TouchableOpacity style={styles.eachProduct} onPress={() => handleOpen(item)}>
                ...
                <View style={{ borderWidth: 0.5, width: "100%" }}>
                    ...
                    {/* <Text style={{ color: "#000", fontSize: 11, marginBottom: 3, textAlign: "center" }}>{item.item_code.length < 22
                        ? `${item.item_code}`
                        : `${item.item_code.substring(0, 20)}...`}</Text> */}
    ...
    useEffect(() => {
        ...
        // setCategoryNameForShow(searchState?.categoryKey || '');
        // setBrandNameForShow(searchState?.brandKey || '');
        // setSearchKeyword(searchState?.productsKey || '');
        // setBrandId(searchState.brandId || '');
        // setCategoryId(searchState.catId || '')
    ...
    //     var priceVal = Number(e);
    //     //console.log("price", priceVal);
    //     let x = parseFloat(priceVal.toFixed(4))
    //     setPrice(x);
    ...
    const validatePriceInput = (input) => {
        // Regex to allow only numbers with up to 4 digits after the decimal
        const validInput = input.match(/^\d*\.?\d{0,4}$/);
        return validInput ? input : price; // If valid, return the input; otherwise, keep the previous value
      };
    ...
    // useEffect(()=> {
    //     console.log('podd', product_qty)
    //     if(product_qty <= 0){
    //         setStringQty("");
    //     }else{
    //         const x = parseInt(product_qty).toFixed(3).toString();
    //         setStringQty(x)
    //     }
    //     const x = parseInt(product_qty);
    //     setStringQty(x)
    ...
    const controlDecimalValue = (number) => {
        // let getLocalStorageData = window.sessionStorage.getItem('persist:root');
        // let parseValue = JSON.parse(getLocalStorageData);
        // let parseMasterData = JSON.parse(parseValue?.master)
        // let decimlCount = parseMasterData?.allMasterData?.systemSetups?.decimal_point;
        let decimlCount = 4;
    ...
    const handleIconPress = () => {
        // Focus the TextInput and select all text
        setIsEditingPrice(true);
        ...
        <SafeAreaView style={{ flex: 1 }}>
            ...
            <View style={styles.mainWrapper}>
                ...
                <View style={{ position: "relative", paddingTop: 22 }}>
                    ...
                    {/* <Text style={[styles.Heading, { fontSize: 16 }]}>Please search</Text> */}
                </View>
                ...
                {/* <ScrollView style={{ flex: 1, marginTop: 30 }}>

                        {
                            searchResults?.map((item, index, arr) => (
                                arr.length > 0 && (
                                    <TouchableOpacity style={styles.eachProduct} key={index} onPress={() => handleOpen(item.item_description, item.item_marketprice, item.pricing, item.item_id)}>
                                        {
                                            item.item_image != "" ? (
            ...
            <View style={[styles.centeredView, { backgroundColor: modalVisible ? "rgba(0,0,0,0.5)" : "transparent", display: modalVisible ? "flex" : "none" }]}>
                ...
                >
                    <View style={{ flex: 1, alignItems: "center", flexDirection: "column", justifyContent: "center" }}>
                        <TouchableWithoutFeedback onPress={() => {setIsEditing(false); setIsEditingPrice(false); Keyboard.dismiss();}} accessible={false}>
                            <KeyboardAvoidingView
                            ...
                            >
                                <View style={styles.modalView}>
                                    ...
                                    {/* <Text style={{ color: "#3d5a80", fontSize: 18, textAlign: "center" }}>AUD {parseFloat(price).toFixed(3)}</Text> */}
                                    {
                                    ...
                                    <View style={{ flexDirection: "row", justifyContent: "center", alignItems: 'center' }}>
                                        <Pressable
                                            disabled={Number(price) >= 1 ? false : true}
                                            style={[styles.button, styles.buttonClose]}
                                            onPress={() => {
                                                dispatch(addToCartFromQuote({
                                                    productData: {
                                                        itemDesc, product_qty, productId, uom, uom_name, totalPrice, price, itemLineDesc
                                                    }
                                                }));
                                                handleClose();
                                                setIsEditing(false);
                                        ...
                                        {/* <TouchableOpacity style={{ marginTop: 12, marginLeft: 10 }} onPress={() => setModalVisible2(true)}>
                                            <Image
            ...
            <View style={[styles.centeredView, { backgroundColor: modalVisible2 ? "rgba(0,0,0,0.5)" : "transparent", display: modalVisible2 ? "flex" : "none" }]}>
                <Modal
                    animationType="fade"
                    transparent={true}
            ...
            <View style={[styles.centeredView, { backgroundColor: categoryModal || brandModal ? "rgba(0,0,0,0.5)" : "transparent", display: categoryModal || brandModal ? "flex" : "none" }]}>
                ...
                >
                    <SafeAreaView style={{ flex: 1, backgroundColor: '#FFF' }}>
                            ...
                            {/* {
                                    categoryState.allCategoryList !== undefined && (
                                        categoryState.allCategoryList.length > 0 && (
                                            ...
                                            isVisible && (
                                                <View style={{ width: "100%", height: 150, position: "absolute", overflow: "hidden", top: "100%", zIndex: 999, left: 0, right: 0, backgroundColor: "#ededed", paddingTop: 5, paddingBottom: 8 }}>
                                                    <ScrollView style={{flex:1}}>
                                                        {
                                                            categoryState.allCategoryList?.map((item, index, arr) => {
                                                                if (arr.length - 1 === index) {
                                                                    return (
                                                                        <TouchableOpacity onPress={() => { setInputVal(item.category_name), setIsvisible(false), setCategoryId(item.category_id), setBtnDisabled(false), setSearchKeyword(" "), console.log("Item name1", item.category_name) }} key={index} style={{ paddingHorizontal: 10, paddingVertical: 5 }}>
                                                                            <Text style={{ color: "#000", fontSize: 12 }}>{item.category_name}</Text>
                                                                        </TouchableOpacity>
                                                                    )
                                                                } else {
                                                                    return (
                                                                        <TouchableOpacity onPress={() => { setInputVal(item.category_name), setIsvisible(false), setCategoryId(item.category_id), setBtnDisabled(false), setSearchKeyword(" "), console.log("Item name1", item.category_name) }} key={index} style={{ paddingHorizontal: 10, paddingVertical: 5, borderBottomColor: "#ccc", borderBottomWidth: 1 }}>
                                                                            <Text style={{ color: "#000", fontSize: 12 }}>{item.category_name}</Text>
                                                                        </TouchableOpacity>
                                                                    )
                                                                }
                        ...
                        {/* <TouchableOpacity style={[styles.btnSubmit, { borderRadius: 8, marginTop: 15 }]} onPress={() => { categoryId == "" ? alert('Please select the category') : setCategoryModal(false) }}>
                                <Text style={styles.textStyle}>Save</Text>
                            </TouchableOpacity> */}
                ...
                >
                    <SafeAreaView style={{ flex: 1, backgroundColor: '#FFF' }}>
                        {/* <TouchableOpacity onPress={() => { setBrandModal(false); setBrandInputVal(""); setBrandId('') }} style={{ position: "absolute", right: -10, top: -10, zIndex: 99, backgroundColor: "#FFF", borderRadius: 40, overflow: "hidden" }}><MaterialCommunityIcons size={35} color="red" name="close-circle" /></TouchableOpacity> */}
                            ...
                            {/* {
                                    brandState.allBrandList !== undefined && (
                                        brandState.allBrandList.length > 0 && (
                                            ...
                                            isVisible2 && (
                                                <View style={{ width: "100%", height:150, position: "absolute", top: "100%", zIndex: 999, left: 0, right: 0, backgroundColor: "#ededed", paddingTop: 5, paddingBottom: 8 }}>
                                                    <ScrollView style={{flex:1}}>
                                                        {
                                                            brandState.allBrandList?.map((item, index, arr) => {
                                                                if (arr.length - 1 === index) {
                                                                    return (
                                                                        <TouchableOpacity onPress={() => { setBrandInputVal(item.brand_name), setIsvisible2(false), setBrandId(item.brand_id), setSearchKeyword(" ") }} key={index} style={{ paddingHorizontal: 10, paddingVertical: 5 }}>
                                                                            <Text style={{ color: "#000", fontSize: 12 }}>{item.brand_name}</Text>
                                                                        </TouchableOpacity>
                                                                    )
                                                                } else {
                                                                    return (
                                                                        <TouchableOpacity onPress={() => { setBrandInputVal(item.brand_name), setIsvisible2(false), setBrandId(item.brand_id), setSearchKeyword(" ") }} key={index} style={{ paddingHorizontal: 10, paddingVertical: 5, borderBottomColor: "#ccc", borderBottomWidth: 1 }}>
                                                                            <Text style={{ color: "#000", fontSize: 12 }}>{item.brand_name}</Text>
                                                                        </TouchableOpacity>
                                                                    )
                                                                }
                        ...
                        {/* {
                            ...
                            bandList?.length > 0 && <ScrollView style={{ flex: 1 }}>
                                {
                                    bandList?.map((item, index, arr) => {
                                        if (arr.length - 1 === index) {
                                            return (
                                                <TouchableOpacity onPress={() => { setBrandInputVal(item.brand_name), setBrandId(item.brand_id); setBrandInputVal(""); setBrandModal(false); setBrandNameForShow(item.brand_name); handleBrandSearch('') }} key={index} style={{ paddingHorizontal: 10, paddingVertical: 8 }}>
                                                    <Text style={{ color: "#000", fontSize: 14 }}>{item.brand_name}</Text>
                                                </TouchableOpacity>
                                            )
                                        } else {
                                            return (
                                                <TouchableOpacity onPress={() => { setBrandInputVal(item.brand_name), setBrandId(item.brand_id); setBrandInputVal(""); setBrandModal(false); setBrandNameForShow(item.brand_name); handleBrandSearch('') }} key={index} style={{ paddingHorizontal: 10, paddingVertical: 8, borderBottomColor: "#ccc", borderBottomWidth: 1 }}>
                                                    <Text style={{ color: "#000", fontSize: 14 }}>{item.brand_name}</Text>
                                                </TouchableOpacity>
                                            )
                                        }
